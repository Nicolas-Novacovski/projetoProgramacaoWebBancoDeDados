...
<head>
    ...
    <script src="js/deleteAndEdit.js"></script>
    <script>
        ...
        if ($conn) {
            ...
            if ($result->rowCount() > 0) {
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo "<button id='btnEditar' onclick='editProduct(" . $row['id_produtos'] . ")'>Editar</button>";
                    echo "<button id='btnDelete' onclick='deleteProduct(" . $row['id_produtos'] . ")'>Deletar</button>";
                    echo "</div>";
                }
            } else {
